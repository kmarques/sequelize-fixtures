...
export class LoadCommand implements yargs.CommandModule {
    ...
    async handler(args: yargs.Arguments): Promise<void> {
        ...
        let dataSource: Sequelize | undefined = undefined;
        try {
            if (withDebug) {
                console.log(chalk.grey('Connection to database...')); // eslint-disable-line
            }
            dataSource = (await import(dataSourcePath)) as Sequelize;
            ...
            for (const fixture of fixturesIterator(fixtures)) {
                try {
                    ...
                    await dataSource.models[fixture.entity].create(entity.dataValues);
                } catch (e) {
                    bar.stop();
                    throw e;
                }
